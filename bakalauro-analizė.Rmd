...
# Update df_filtered with the final filtered dataset
...
df_psm1_d1 <- df_psm1 %>%
  ...
  mutate(divlength_group = case_when(
    ...
    divlength == 1 ~ "1-2",
    divlength == 2 ~ "2-3",
    TRUE ~ NA_character_  # fallback if missing
  )) %>%
...
library(Matching)
library(doParallel) # Kadangi ilgai užtrunka, paraleliai naudojame kelis kompiuterio cores
...
p = p +
    labs(
    title = "Kontrolinių kintamųjų balansas prieš ir po poravimo",
    x = "Absoliutūs standartizuoti vidurkių skirtumai",
    y = NULL
  ) +
  scale_color_manual(
    name = "",  # Legend title
    labels = c("Prieš poravimą", "Po poravimo"),  # Legend values
...
df_plot1 <- df_psm3 %>%
  mutate(
    ...
    observation = factor(observation, levels = c(1, 2), labels = c("Prieš skyrybas", "Po skyrybų"))
  ) %>%
  bind_rows(
    df_psm3 %>%
      mutate(
...
plot1 <- ggplot(df_plot1, aes(x = gender, y = casp, fill = divorced)) +
  ...
  labs(
    title = "Psichologinės gerovės skirtumai tarp \n išsiskyrusiųjų ir likusių santuokoje",
    x = NULL,
    y = "CASP-12",
...
```{r}
# Sukuriame diagramą 2 užd
plot2 <- ggplot(df_psm3, aes(x = as.factor(observation), y = casp, 
                                  color = as.factor(divorced), 
                             group = as.factor(divorced))) +
  stat_summary(fun = mean, geom = "line", size = 1) +
  stat_summary(fun = mean, geom = "point", size = 2) +
  labs(
    title = "Psichologinės gerovės pokytis išsiskyrusiųjų\n ir likusių santuokoje grupėse",
    x = NULL,
    y = "CASP-12",
    color = "Skyrybų statusas"
  ) +
  scale_color_manual(
    labels = c("0" = "Liko santuokoje", "1" = "Išsiskyrė"),
    values = c("0" = "#80dfff", "1" = "#ff8080")
  ) +
  scale_x_discrete(labels = c("1" = "Prieš skyrybas", "2" = "Po skyrybų")) +
  theme_minimal(base_size = 10, base_family = "Helvetica") +
  theme(
    plot.title = element_text(size = 14, face = "bold", hjust = 0.5, margin = margin(b = 10)),
    axis.text = element_text(size = 10),
  ...
  scale_y_continuous(limits = c(12, 48), breaks = c(12, 20, 30, 40, 48))

plot2
ggsave("plot2.svg", plot = plot2, width = 6.3, height = 4.5, units = "in", dpi = 300)
```
...
df_plot5 <- df_anova %>%
  ...
  mutate(
    ...
    divlength_group = factor(divlength_group, levels = c("3-2", "2-1", "1-0", "0-1", "1-2", "2-3"), labels = c("3-2 m.", "2-1 m.", "1-0 m.", "0-1 m.", "1-2 m.", "2-3 m."))
  )
...
plot5 <- ggplot(df_plot5, aes(x = divlength_group, y = casp, color = gender, group = gender)) +
  ...
  labs(
    title = "Psichologinės gerovės skirtumai priklausomai \nnuo laiko iki skyrybų ir laiko po skyrybų",
    x = NULL,
    y = "CASP-12",
    color = "Lytis"
  ) +
